  ...
  useEffect(() => {
    async function load() {
      const [groupRes, memberRes] = await Promise.all([
        ...
        user
          ? supabase.from('community_members').select('id').eq('community_id', id).eq('user_id', user.id).maybeSingle()
          : Promise.resolve({ data: null }),
      ]);
      setGroup(groupRes.data as Community | null);
  ...
  if (!group) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-8 text-center">
        <p className="text-white/40">Group not found.</p>
        <Link href="/dashboard/groups" className="text-emerald-400 text-sm hover:underline mt-2 inline-block">
          Back to Groups
        </Link>
      </div>
    );
      ...
      <div className={`glass-card p-6 mb-6 ${meta.bg} border border-white/[0.08]`}>
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {group.description && (
              <p className="text-sm text-white/60 leading-relaxed mb-4">{group.description}</p>
            )}
            <button
              onClick={toggleMembership}
              disabled={toggling}
              className={joined ? 'btn-ghost text-xs px-4 py-2 hover:text-rose-400' : 'btn-brand text-xs px-4 py-2'}
            >
              {toggling ? '…' : joined ? 'Leave Group' : <><Plus className="w-3.5 h-3.5" />Join Group</>}
            </button>
          </div>
        </div>
      </div>

      <Feed scope="global" communityId={group.id} />
    </div>
  );
}
